...
pub fn draw_text_top_center<T: AsRef<str>>(content: T, x: i32, y: i32) {
    ...
    if total_lines == 1 {
        let x = x - ((content.as_ref().len() as i32 * SINGLE_CHAR_PIXEL as i32) / 2);
        ...
        content
            ...
            .enumerate()
            .for_each(|(idx, line)| {
                let x = x - ((line.len() as i32 * SINGLE_CHAR_PIXEL as i32) / 2);
                let y = y + (idx as i32) * (SINGLE_CHAR_PIXEL as i32 + 2);
                draw_text(line, x, y);
            });
    }
}
